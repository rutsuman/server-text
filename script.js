...
// Handle click on preview image to open fullscreen
function handlePreviewClick(e) {
  ...
  if (questId && studentWorks[questId]) {
    ...
    // Open fullscreen with the work image and info
    openFullscreenFromWork(work, quest);
  } else {
    // If no saved work data, just show the image without details
...
// Open fullscreen from work overlay (with full details)
function openFullscreenFromWork(work, quest) {
  ...
  // Set title - use work title or quest title
  ...
  // Store current work for potential edit link
...
// Simple fullscreen for when there's no saved work data
...
// Check visibility
      if (id === "gallery" || id === "pathfinder") { 
        // For maps 2 and 3, hide the map-linked hotspots and show floating nav
        if (currentMap === "map1") {
          // On map1: show map-linked hotspots, hide floating nav
          hotspot.style.display = "block";
          if (floatingNav) floatingNav.style.display = "none";
        } else {
          // On maps 2 and 3: hide map-linked hotspots, show floating nav
...
function openWorkOverlay(questId) {
  ...
  if (preview) {
    ...
    // ========== ADD THIS: Make preview clickable ==========
...
function closeWorkOverlay() {
  ...
  overlay.style.display = "none";
    ...
    if (questId && typeof openQuestOverlay === 'function') {
    openQuestOverlay(questId);
  } else if (typeof openQuestOverlay === 'function') {
    // Fallback: open quest overlay without specific ID
    openQuestOverlay();
  }
}
...
function renderGalleryItems() {
    ...
    worksArray.forEach(([questId, work]) => {
        // Skip if work has no title and no image and no description (empty work)
        ...
        // Create thumbnail container with wrapper
        ...
        if (work.image) {
            ...
            thumbnail.addEventListener("click", (e) => {
                e.stopPropagation(); // Prevent opening the quest
        ...
        // Create size and media info (optional)
        ...
        // Add click event to open quest (when clicking on the item, not the image)
        galleryItem.addEventListener("click", (e) => {
            // Don't open quest if clicking on the image (it's handled separately)
...
function initializeFullscreenViewer() {
    ...
    // Close on overlay click (clicking outside image)
    overlay.addEventListener("click", (e) => {
        if (e.target === overlay) {
            closeFullscreenImage();
        }
...
function openFullscreenImage(work, quest) {
    ...
    // Store the esc handler on the overlay to remove later
    overlay.escHandler = escHandler;
}
